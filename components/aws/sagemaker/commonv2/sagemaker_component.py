...
class SageMakerComponent:
        ...
        try:
            while True:
                ...
                if ack_resource_meta:
                    ...
                    if arn is not None:
                        resource_ack = (
                            f"Created Sagemaker job with ARN: {arn}"
                            if not self.resource_upgrade
                            else f"Submitting update for Sagemaker resource with ARN: {arn}"
                        )
                ...
                if arn:
                    if self.resource_upgrade:
                        ...
                        if (
                            current_condition_times
                            == self.initial_resouce_condition_times
                        ):
    ...
    def _wait_resource_consumed_by_controller(
        self, wait_periods, period_length,
    ):
        ...
        if not self._get_resource_exists():
            logging.error(
                f"Resource %s does not exist", (self.job_name),
            )
            return None
    ...
    def _create_resource(
        self, cr_spec: object, wait_periods=6, period_length=10,
    ):
    ...
    def _get_condition_times(self):
        ...
        resource_conditions = self._get_resource()["status"]["conditions"]
        condition_times = [
            condition.get("lastTransitionTime", None)
